...
include_once(PY_ROOT . DIR_SEP . 'system' . DIR_SEP . 'interfaces' . DIR_SEP
		. 'SystemInterface.php'); # System Interface
include_once(PY_ROOT . DIR_SEP . 'system' . DIR_SEP . 'traits' . DIR_SEP
...
class System implements \SystemInterface {
	...
	private $CLASS_TYPES = array('CLASS', 'INTERFACE', 'MODEL', 'TRAIT');
	private $OUTPUT_TYPES = array("php", "html", "html5", "json",);
	...
	function __autoload($class) {
		...
		$filename = (DIR_SEP != "\\") ? str_replace("\\", DIR_SEP, $class)
				: $class;
		...
		foreach ($paths AS $path) {
			...
			foreach ($extensions AS $ext) {
				...
				if (is_file(
						$file = PY_ROOT . DIR_SEP . $path . DIR_SEP . $filename
								. $ext)) {
					require($file);
					...
					# without a namespace dir
						...
						$file = PY_ROOT . DIR_SEP . $path . DIR_SEP
	...
	protected function __initialize() {
		...
		$this->TYPE_COUNT = count(
				preg_grep_keys("/^TYPE_/i", $reflection->getConstants()));
		...
		# register autoloader
		$this->autoload = array($this, '__autoload');
		...
		# setup hooks
		$this->service()
				->prepare("hook", '\HookRegistryInterface', new HookRegistry());
	...
	private function readStructure($return = false) {
		...
		foreach ($this->CLASS_TYPES AS $type) {
			$PY["{$type}_PATH"] = (isset($PY["{$type}_PATH"])) ? $PY["{$type}_PATH"]
					: []; # class paths
			$PY[$type] = (isset($PY[$type])) ? $PY[$type] : []; # class list
		}
		$PY['TEMPLATE_PATH'] = (isset($PY['TEMPLATE_PATH'])) ? $PY['TEMPLATE_PATH']
				: []; # core templates
		...
		foreach (scandir(PY_ROOT) AS $mod) {
			...
			if (is_file($confFile = $path . DIR_SEP . "module.json")) {
				# load module config
				$PY['MODULES'][$mod] = $conf = json_decode(
						file_get_contents($confFile), true);
				# cache requirements
				$typeName = strtoupper(
						isset($conf['type']) ? $conf['type'] : 'module');
				$type = (defined(__CLASS__ . "::TYPE_$typeName")) ? constant(
								__CLASS__ . "::TYPE_$typeName")
						: static::TYPE_MODULE;
				$load[$type][$mod] = $mod == static::SYSTEM ? []
						: [static::SYSTEM];
				if (isset($conf['require'])) {
					if (is_array($conf['require'])) {
						$load[$type][$mod] = array_merge($load[$type][$mod],
								$conf['require']);
				...
				foreach ($this->CLASS_TYPES AS $type) {
					$dir = isset(
							$conf[$cnf_name = (strtolower($type) . "_dir")]) ? $conf[$cnf_name]
							: constant("static::DIR_{$type}");
					if (is_dir(PY_ROOT . DIR_SEP . $mod . DIR_SEP . $dir)) {
						array_push($PY["{$type}_PATH"], $mod . DIR_SEP . $dir);
						if (substr($path = $mod . DIR_SEP . $dir, -1)
								== DIR_SEP)
							$path = substr($path, 0, -1);
						foreach (new \RecursiveIteratorIterator(
								new \RecursiveDirectoryIterator(
										PY_ROOT . DIR_SEP . $path,
										\FilesystemIterator::SKIP_DOTS)) as $filePath => $fileObj) {
							if (array_search(
									"."
											. pathinfo($filePath,
													PATHINFO_EXTENSION),
									$extensions) !== false) {
								$file = str_replace(
										PY_ROOT . DIR_SEP . $path . DIR_SEP,
										"", $filePath);
								if (!isset(
										$PY[$type][$base = substr($file, 0,
												strpos($file, "."))])) {
				...
				$template_dir = isset($conf['template_dir']) ? $conf['TEMPLATE_DIR']
						: static::DIR_TEMPLATE;
				if (is_dir(PY_ROOT . DIR_SEP . $mod . DIR_SEP . $template_dir)) {
					array_push($PY['TEMPLATE_PATH'],
							$mod . DIR_SEP . $template_dir);
					if (substr($path = $mod . DIR_SEP . $template_dir, -1)
							== DIR_SEP)
						$path = substr($path, 0, -1);
					foreach (new \RecursiveIteratorIterator(
							new \RecursiveDirectoryIterator(
						...
						if (array_search(
							...
							if (!isset(
									$PY["TEMPLATES"][$base = substr($file, 0,
											strpos($file, "."))])) {
								$PY["TEMPLATES"][$base] = $path . DIR_SEP
										. $base;
							}
						}
					}
